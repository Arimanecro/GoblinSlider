...
window.addEventListener("load", function() {
  ...
  const listAnimations = [
    document.getElementById("gear_sm1"),
    document.getElementById("gear_sm2"),
    document.getElementById("gear_bg"),
    goblinFall
  ];
  ...
  window.addEventListener("visibilitychange", () => {
    return !document.hidden
      ? (document.getElementById("WomenSprite").style.animationPlayState =
          "running")
      : (document.getElementById("WomenSprite").style.animationPlayState =
          "paused");
  });
  ...
  pic.addEventListener("change", e => {
    if (e.target.files.length > 3) {
      ...
      const three = document.querySelectorAll(
        "#wagon_container > div.wagon.img"
      );
      [...three].forEach(v => (v.style.cursor = "pointer"));
    } else {
      slider.incorrectCountImages();
    }
  });
  ...
  btn_close.addEventListener(
    "click",
    () => (slider.curtain.style.left = "-73%")
  );

  goblin.addEventListener(
    "animationend",
    function() {
    ...
    function() {
      let timeout = setInterval(function() {
        ...
        if (typeof x !== "undefined") {
          window.scrollTo(x.slice(0, -1), x.slice(0, -1));
          //   window.scrollTo({
          //     top: x.slice(0, -1),
          //     behavior: "smooth"
          //   });
  ...
  goblinFall.addEventListener(
    ...
    function() {
      ...
      board.style.top = "0px";
      setTimeout(() => window.scrollTo({ top: 3060, behavior: "smooth" }), 500);
    },
    false,
    false,
    "fall"
  ...
  down.addEventListener("transitionend", () =>
  ...
  function btnEvent(name, btn) {
    if (document.getElementById("wagon_container").children.length > 3) {
      ...
      startPauseAnimation(
        name,
        listAnimations,
        window.getComputedStyle(btn_right).getPropertyValue("transform")
      );
      wheels.forEach(s => {
        if (name == "btn_left") {
      ...
      img.addEventListener("transitionend", () => {
        ...
        if (
          matrixRotate(
            window.getComputedStyle(btn).getPropertyValue("transform")
          ) == 90
        ) {
  ...
  function startPauseAnimation(btn, arr, cssTransform) {
    if (matrixRotate(cssTransform) == 0) {
      if (btn == "btn_right") {
        ...
        btn_left.style.transform = "rotate(-90deg)";
      }
    }
