export default class GoblinSlider {
  constructor({ display = 3, widthChild = 223 } = {}) {
    ...
    this.wrapper.setAttribute(
      "style",
      "width:" + this.countChildWrapper * this.widthChild + "px"
    );

    this.arrow_box = document.querySelector(".arrow_box");
    this.arrow_box.addEventListener(
      "animationend",
      () => (this.arrow_box.style = "")
    );
  ...
  move(direction) {
    if (this.wrapper.children.length > 3) {
      ...
      if (direction == "forward") {
        ...
        for (let n of add) {
          ...
          this.wrapper.insertBefore(
            element,
            this.wrapper.lastElementChild.nextSibling
          );
        }
      } else {
        this.next.disabled = true;
      ...
      this.wrapper.firstElementChild.setAttribute(
        "style",
      ...
      this.wrapper.setAttribute(
        "style",
        "width:" + this.wrapper.childElementCount * this.widthChild + "px;"
      );
  ...
  transition(direction) {
    let c = 0;
    this.wrapper.firstElementChild.addEventListener(
      "transitionend",
      () => {
        ++c;
  ...
  incorrectCountImages() {
    return (this.arrow_box.style.animation =
      "shake 0.82s cubic-bezier(.36,.07,.19,.97) both running");
  }
    ...
    document.querySelectorAll("div.wagon.img").forEach(img => {
      img.addEventListener(
        ...
        () => {
          ...
          this.curtain_link.setAttribute(
            "href",
            this.clearPathImg(img.children[0].style.backgroundImage)
          );
          this.curtain_img.setAttribute(
            "src",
            this.clearPathImg(img.children[0].style.backgroundImage)
          );
        },
